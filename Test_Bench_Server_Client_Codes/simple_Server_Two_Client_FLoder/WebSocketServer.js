...
//var storeNewIncomingClientArray = new Array();
...
function oHandleTextMessage(inConnectionID, inMessage){
	...
	console.log("Incoming message from client : " + incomingMessage); //parsed message
...
/******Exchange message between the dynamically between the clients even though connections are refreshed******/
	...
	case '000' :  	/* Handle simple communication handshake */
					...
					oSendAckBackToClient(inConnectionID, inAction, responseJSONMessage);
				break;
	case '180':	/* Handle simple communication handshake */
				var responseJSONMessage = {action:"aaaaaa"}
				oSendAckBackToClient(sendingClientId, inAction, responseJSONMessage);
	break;
	default :
				break;
	}
...
function oSendAckBackToClient(inConnectionID, inAction, inresponseJSONMessage){
	...
	oSendToConnection(inConnectionID, JSON.stringify(inresponseJSONMessage));
}

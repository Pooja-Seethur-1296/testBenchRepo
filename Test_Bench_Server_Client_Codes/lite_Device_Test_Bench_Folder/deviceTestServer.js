...
function oHandleTextMessage(inConnectionID, inMessage){
    ...
    inStatus = incomingMessage.state;                  //ask doubt
    ...
    var words = fs.readFileSync('noOfIteration.json', 'utf8');
    console.log(words);
    var inNoOfIterations = readIterationNumber(words);
    console.log(inNoOfIterations);
...
/**********Response handling from device and sending information back to user/client****************/
    ...
    switch(inResp){
        ...
        case '101': console.log("Response from the device received");
                    var incomingResponse = deviceResponseToAppObj.oSendDeviceCurrentInfo(inDeviceId,inStatus,inMode);
        ...
        case '103': console.log("Mode Change Response from the device received");
                    var incomingResponse = deviceResponseToAppObj.oModeChangeInfo(inDeviceId,inStatus,inMode);
                    oSendAckBackToClient(sendToOtherClient, outNoOfIterations, incomingResponse);
                    var report = 'Mode change acknowlwdgement is sent to client: Test cycle completed';
                    writeReportToFile(inResp,respCounter,report);
                    break;

        case '102': console.log("Manual mode On/Off response received");
                    var incomingResponse = deviceResponseToAppObj.oManualOnOrOff(inDeviceId,inStatus,inFlow,inFlowRate);
                    oSendAckBackToClient(sendToOtherClient, outNoOfIterations, incomingResponse);
        ...
        case '105': console.log("Mode Change Response from the device received");
                    var incomingResponse = deviceResponseToAppObj.oSetTimerSheet(inResp,inDeviceId,inTs);
                    oSendAckBackToClient(sendToOtherClient, outNoOfIterations, incomingResponse);
                    var report = 'Timer Sheet is received by the device';
                    writeReportToFile(inResp,respCounter,report);
                    break;
        ...
        case '104': console.log("Mode Change Response from the device received");
                    var incomingResponse = deviceResponseToAppObj.oSetTimerSheet(inResp,inDeviceId,inTs);
                    oSendAckBackToClient(sendToOtherClient, outNoOfIterations, incomingResponse);
                    var report = 'Timer Sheet (Cron job) is received by the device';
                    writeReportToFile(inResp,respCounter,report);
                    break;
    default :
                    break;
